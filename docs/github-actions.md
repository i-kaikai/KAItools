...
- 桌面更新的公网验签会优先通过 HTTP/2 自动协商；服务端不支持时使用 HTTP/1.1 长连接池，并保持 16 个
  并发 worker。对象按流式 SHA-256 校验，不会完整读入内存。出现可恢复的传输或资源错误时，仅对失败对象
  以 2 并发重试。
- 发布前会保存上一版公开 `latest.json` 及签名；发布后会重新验证新旧元数据与清单签名，仅下载
  并校验条目发生变化的对象。首次发布或上一版元数据、清单无法使用时，自动回退为全量校验。
- 桌面发布完成并切换 `current` 后会清理旧发布目录、旧版清单、未被当前清单引用的摘要对象，以及符合发布
  临时命名规则的遗留下载文件。该策略不保留服务器端桌面更新回滚窗口。
...
# Optional: concurrent HTTP range requests for one Artifact ZIP (default: 10; max: 10).

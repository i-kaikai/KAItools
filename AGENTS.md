...
- 工作区根目录不是 Git 仓库；Git 状态、分支、提交、拉取和推送只在本仓库中执行。相邻
  的 `kaitools-api` 是独立仓库，不要把其源码或提交混入本仓库。
- 后续 Git commit message 必须使用中文，并准确概括本次变更内容。
- 涉及前后端接口契约的变更，必须分别在两个仓库实现和验证；客户端配置 API origin 时只
  保存源地址，路径由统一客户端拼接。

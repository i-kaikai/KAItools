...
if (-not $PortableRoot) {
    $PortableRoot = Join-Path (Split-Path -Parent $PSScriptRoot) 'dist\DevToolkit'
}
$Executable = Join-Path $PortableRoot 'DevToolkit.exe'
...
$InitialProcessIds = @(Get-Process -Name 'DevToolkit' -ErrorAction SilentlyContinue | ForEach-Object Id)
...
try {
    ...
    $NewProcesses = @(
        Get-Process -Name 'DevToolkit' -ErrorAction SilentlyContinue |
            Where-Object { $InitialProcessIds -notcontains $_.Id }
    )
    $WindowProcess = $NewProcesses | Select-Object -First 1
    if (-not $WindowProcess) { throw 'DevToolkit did not create a live process' }
    $Started = $false
    if (Test-Path -LiteralPath $LogPath) {
    ...
    if (-not $Started) {
        throw 'DevToolkit process is alive but the main WebView2 window did not start'
    }
